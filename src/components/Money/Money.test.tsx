...
describe('Money', () => {
	 ...
	 *
	 * Компонент Money
	 * Пример использования: <Money value={5000.99} currency='USD' />
	 * Параметр: value - число, может быть дробным
	 * Параметр: currency - тип валюты, "RUB", "EUR", "GBP", "USD"
	 * Маска для валют RUB = ₽, EUR = €, GBP = £, USD = $
	 * Особенности: Нужно соблюсти формат вывода. К примеру, мы передаем value = 4000.45, currency = RUB - Получаем <span><span>4000</span><span>,45</span><span>₽</span></span>
	 */
	describe('Базовые состояния', () => {
		it('Рендерим доллары', () => {
			...
			expect(component.html()).toEqual(
				'<span><span>5000</span><span>,99</span><span>$</span></span>'
			);
		});
		...
		it('Рендерим Евро', () => {
			...
			expect(component.html()).toEqual(
				'<span><span>600</span><span>,39</span><span>€</span></span>'
			);
		});
		...
		it('Рендерим Фунты', () => {
			...
			expect(component.html()).toEqual(
				'<span><span>100</span><span>,69</span><span>£</span></span>'
			);
		});
		...
		it('Рендерим Рубли', () => {
			...
			expect(component.html()).toEqual(
				'<span><span>40</span><span>,99</span><span>₽</span></span>'
			);
		});
	});
	...
	describe('Проверка коректности работы отбражения значений', () => {
		it('Нет копеек', () => {
			...
			expect(component.html()).toEqual(
				'<span><span>40</span><span>₽</span></span>'
			);
		});
